...
if (isset($_POST['login'])) {
    ...
    $db = new Logics();
    $message = $db->login($email, $password);
}
?>
...
<body class="dark-grid-background flex items-center justify-center min-h-screen">
    ...
    <div class="w-full max-w-md p-6 sm:p-8 bg-gray-900 rounded-xl shadow-2xl card-glow backdrop-blur-sm bg-opacity-80">
        ...
        <h2 class="text-3xl font-bold text-white text-center mb-2">User Login</h2>
        <p class="text-gray-400 text-center mb-8">
            Access your control panel.
        </p>

        <?php if ($message) {
            echo "<p class='bg-red-900 border border-red-700 text-red-300 px-4 py-3 rounded relative mb-4 text-sm' role='alert'>$message</p>";
        } ?>
        ...
        <form method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input type="password" name="password" id="password" placeholder="••••••••" required class="w-full px-4 py-2 bg-gray-800 text-white border border-gray-700 rounded-lg placeholder-gray-500 
               focus:outline-none focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 transition">
                <div class="text-right mt-2">
                    <a href="./forgot-password.php" class="text-sm text-indigo-400 hover:text-indigo-300 transition">
                        Forgot Password?
                    </a>
                </div>
            </div>
        ...
        <p class="mt-8 text-center text-sm text-gray-400">
            ...
            <a href="./registration.php"
                class="text-indigo-400 hover:text-indigo-300 font-medium transition duration-300">
                Register here
            </a>
        </p>
    </div>
